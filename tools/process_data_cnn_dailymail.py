...
INSTRUCTION = "Summarize the news in no more than one sentence. The summary should be as short as possible, not re-explaining the news in detail."
...
class Encoder(object): 
    ...
    def encode(self, line):
        ...
        prompt = template.format(instruction=INSTRUCTION, input=line["article"])
        
        response = line["highlights"]
...
def main():
    ...
    if args.dev_num > 0:
        ...
        all_data = {
            ...
            "train": [{key: value[i] for key, value in train_dict.items()} for i in range(len(train))]
        }
    else:
        all_data = {
            "train": [{key: value[i] for key, value in train_dict.items()} for i in range(len(train))]
